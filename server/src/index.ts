...
// Start context sync — watches ~/johns-command-center/context/ for updates
const contextSync = new ContextSync((update) => {
  const room = OfficeRoom.getActiveRoom();
  if (!room) return;
  // Inject context update into relevant agent memories via the room
  room.injectContextUpdate(update);
});
contextSync.start();
